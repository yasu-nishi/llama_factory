...
@dataclass
class Ll_jaTemplate:
    format_user1: "Formatter"
    format_user2: "Formatter"
    format_assistant: "Formatter"
    format_system: "Formatter"
    format_function: "Formatter"
    format_observation: "Formatter"
    ...
    def _encode(
        ...
        for i, message in enumerate(messages):
            ...
            if message["role"] == Role.USER.value and i == 1:
                elements += self.format_user1.apply(content=message["content"], idx=str(i // 2))
            elif message["role"] == Role.USER.value:
                elements += self.format_user2.apply(content=message["content"], idx=str(i // 2))
            elif message["role"] == Role.ASSISTANT.value:
                elements += self.format_assistant.apply(content=message["content"])
            elif message["role"] == Role.OBSERVATION.value:
                elements += self.format_observation.apply(content=message["content"])
...
def _register_template(
    ...
    template_class = Llama2Template if name.startswith("llama2") LljaTemplate elif name.startswith("ll_ja")else Template
...
_register_template(
    name="ll_ja",
    format_user1=StringFormatter(slots=["\n\n### 入力:{{content}}\n\n### 応答:\n"]),
    format_user2=StringFormatter(slots=[{"bos_token"}, "\n\n### 入力:{{content}}\n\n### 応答:\n"]),
    format_system=StringFormatter(slots=[{"bos_token"}, "{{content}}"]),
    default_system=(
